...
export default function TeacherDashboard() {
  ...
  const createQuiz = async () => {
    ...
    try {
      ...
      setQuizzes([...quizzes, { id: docRef.id, title: quizTitle, questions, teacherId: user.uid }]);
      setQuizTitle('');
      setQuestions([]);
    } catch (e: any) {
      console.error(e);
    ...
    return (
      <div className="min-h-screen bg-[#0A0A0B] text-[#E4E4E7] p-8 flex flex-col items-center justify-center">
        <div className="max-w-md w-full bg-[#141416] p-10 rounded-2xl border border-[#27272A] text-center shadow-2xl">
          ...
          <button 
            onClick={login}
            className="w-full bg-white text-black font-black py-4 rounded-xl hover:bg-gray-200 transition-colors flex items-center justify-center gap-3"
          >
    ...
    <div className="min-h-screen bg-[#0A0A0B] text-[#E4E4E7] p-8 font-sans">
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={logout} className="text-red-500 hover:text-red-400 font-bold uppercase text-xs tracking-widest transition-colors">
            Sair da Conta
          </button>
        </div>
